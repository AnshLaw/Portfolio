...
test('experience timeline follows scroll and jumps from the ruler', async ({ page }) => {
  const errors = []
  page.on('pageerror', error => errors.push(error.message))
  await page.setViewportSize({ width: 1440, height: 900 })
  await page.goto('/experience/')
  await expect(page.locator('.work-entry')).toHaveCount(3)
  await expect(page.locator('.ruler-track button')).toHaveCount(3)
  await page.getByRole('button', { name: 'Jump to Hyundai Mobis' }).click()
  await expect(page.locator('.work-entry').nth(1)).toHaveAttribute('data-active', 'true')
  await expect(page.locator('.work-entry').nth(1).locator('mark').first()).toBeVisible()
